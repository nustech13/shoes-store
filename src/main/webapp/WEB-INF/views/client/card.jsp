			...
			<tbody>
				<c:forEach var="item" items="${sessionScope['scopedTarget.cardService'].items}">
					<tr data-id="${item.product.id}"
						...
						<td class="col-sm-1 col-md-1 text-center"><strong><fmt:setLocale value="vn"/><fmt:formatNumber value="${item.product.price*(1-item.product.sale)}"/> VNĐ</strong></td>
						<td class= "total col-sm-1 col-md-1 text-center">
								<fmt:setLocale value="vn"/><fmt:formatNumber value="${item.totalPrice}"/> VNĐ
						</td>
				...
				<tr >
					...
					<td><b id="amt"><fmt:setLocale value="vn"/><fmt:formatNumber value="${card.amount}"/> VNĐ</b></td>
					<td>
	 ...
	 $("tr[data-id] input").on("input",function(){
		...
		$.ajax({
			...
			success: function( response ){
				$("#amt").html(response[1]+' VNĐ');
				$("#cnt").html(response[0]);
			}
		});
		var totalPrice = price*(1-sale)*qty;
		$(this).closest("tr").find("td.total").html(totalPrice+ ' VNĐ');
	 });
